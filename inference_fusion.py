...
用于测试和部署融合模型
python inference_fusion.py --checkpoint runs/visdrone_fusion_v2/stage2_best.pth --input test_images/hazy_road2.jpg --output output.jpg --model-size s --conf 0.25
"""
...
class FusionInference:
    ...
    def detect(self, img_path: str, conf_threshold: float = 0.05, save_path: str = None, dehaze_prompt: str = None):
        """
        对单张图像进行检测
        Args:
            img_path: 图像路径
            conf_threshold: 置信度阈值 (默认降低到 0.05 以适应雾天场景)
            save_path: 结果保存路径
        ...
        if dehaze_prompt:
             ...
             # 保存一下纯去雾结果看看
             if save_path:
                 debug_dehazed_path = save_path.replace('.jpg', '_debug_dehazed.jpg')
                 cv2.imwrite(debug_dehazed_path, img_dehazed_bgr)
                 print(f"DEBUG: 纯去雾图已保存至 {debug_dehazed_path} (请检查此图是否灰蒙蒙)")
    ...
    def parse_results(self, results, conf_threshold: float = 0.05, orig_shape=None):
        """解析YOLO输出"""
    ...
    def detect_video(self, video_path: str, output_path: str, conf_threshold: float = 0.25, dehaze_prompt: str = None):
        ...
        while cap.isOpened():
            ...
            # 可视化结果并写入视频 (使用去雾后的 BGR 图像)
            vis_frame = self.visualize(img_dehazed_bgr, detections)
            out.write(vis_frame)
...
def main():
    ...
    # 自动检查文件后缀判定是否为视频
    is_video = args.video
    if not is_video and args.input.lower().endswith(('.mp4', '.avi', '.mov', '.mkv')):
        print(f"检测到视频文件后缀: {args.input}，自动切换为视频模式。")
        is_video = True

    # 执行检测任务
    if is_video:
        # 如果没有指定视频后缀，默认mp4
        output_path = args.output if args.output.endswith(('.mp4', '.avi', '.mov')) else args.output + '.mp4'
